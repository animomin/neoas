...
(function (factory) {
  ...
  var button = renderer.create('<button type="button" class="note-btn btn btn-default btn-sm">', function ($node, options) {
    if (options && options.tooltip) {
      ...
      if(!options.customClass || options.customClass == ""){
        options.customClass = "note-btn btn btn-default btn-sm";
      }else{
        options.customClass = "note-btn btn btn-sm " + options.customClass;
      }

      if(options.buttonID){
        $node.attr({id : options.buttonID});
      }

      if(options.customAttr){        
        $node.attr(options.customAttr.attrName, options.customAttr.attrValue);
      }

      $node.attr({
        title: options.tooltip,
        class : "note-btn btn btn-sm " + options.customClass
      }).tooltip({
        container: 'body',
        trigger: 'hover',
        placement: options.tooltipPosition || 'bottom'
      });
    }
  });
  ...
  var Placeholder = function (context) {
    ...
    this.initialize = function () {
      ...
      }).html(options.placeholder).prependTo($editingArea);
    };
